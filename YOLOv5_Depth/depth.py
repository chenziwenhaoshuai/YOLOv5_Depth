...
try:
    while True:
        ...
        depth_image = np.asanyarray(depth_frame.get_data())#此处转换为数组，宽度高度为图像宽度高度，数值最小为0，最大为65535，初步估计单位为毫米
        ...
        depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha=0.03), cv2.COLORMAP_JET)#  有效距离(mm)*alpha=255
        #depth_colormap = cv2.applyColorMap(color_image, cv2.COLORMAP_JET)
